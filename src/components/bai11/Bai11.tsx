...
const Bai11: React.FC = () => {
  ...
  const [showDeleteCompletedModal, setShowDeleteCompletedModal] = useState<boolean>(false);
  ...
  const deleteCompletedTasks = async () => {
    ...
    try {
      ...
      await Promise.all(completedTasks.map(task => axios.delete(`https://jsonplaceholder.typicode.com/todos/${task.id}`)));
    ...
    <div className="app-container">
      ...
      {loading ? (
        ...
        <>
          ...
          <button onClick={() => setShowDeleteCompletedModal(true)}>Xóa công việc hoàn thành</button>
        </>
      )}
      ...
      {showDeleteCompletedModal && (
        <div className="modal">
          <div className="modal-content">
            <h2>Xác nhận xóa</h2>
            <p>Bạn có chắc chắn muốn xóa tất cả các công việc hoàn thành?</p>
            <button onClick={() => setShowDeleteCompletedModal(false)}>Hủy</button>
            <button onClick={deleteCompletedTasks}>Xác nhận</button>
          </div>
        </div>
      )}
    </div>
