...
const Bai12: React.FC = () => {
  ...
  const deleteAllTasks = async () => {
    ...
    try {
      ...
      await Promise.all(taskIds.map(id => axios.delete(`https://jsonplaceholder.typicode.com/todos/${id}`)));
      setTasks([]);
    ...
    <div className="app-container">
      ...
      {loading ? (
        ...
        <>
          ...
          <button onClick={() => setShowDeleteAllModal(true)}>Xóa tất cả công việc</button>
        </>
      )}
      ...
      {showDeleteAllModal && (
        <div className="modal">
          <div className="modal-content">
            <h2>Xác nhận xóa</h2>
            <p>Bạn có chắc chắn muốn xóa tất cả các công việc?</p>
            <button onClick={() => setShowDeleteAllModal(false)}>Hủy</button>
            <button onClick={deleteAllTasks}>Xác nhận</button>
          </div>
        </div>
      )}
    </div>
